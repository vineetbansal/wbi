function varargout = Worm3DFiducialPickerNetwork_2020(varargin)
% WORM3DFIDUCIALPICKERNETWORK_2020 MATLAB code for Worm3DFiducialPickerNetwork_2020.fig
%      WORM3DFIDUCIALPICKERNETWORK_2020, by itself, creates a new WORM3DFIDUCIALPICKERNETWORK_2020 or raises the existing
%      singleton*.
%
%      H = WORM3DFIDUCIALPICKERNETWORK_2020 returns the handle to a new WORM3DFIDUCIALPICKERNETWORK_2020 or the handle to
%      the existing singleton*.
%
%      WORM3DFIDUCIALPICKERNETWORK_2020('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WORM3DFIDUCIALPICKERNETWORK_2020.neuron6 with the given input arguments.
%
%      WORM3DFIDUCIALPICKERNETWORK_2020('Property','Value',...) creates a new WORM3DFIDUCIALPICKERNETWORK_2020 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Worm3DFiducialPickerNetwork_2020_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Worm3DFiducialPickerNetwork_2020_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Worm3DFiducialPickerNetwork_2020

% Last Modified by GUIDE v2.5 01-May-2020 21:48:32

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @Worm3DFiducialPickerNetwork_2020_OpeningFcn, ...
    'gui_OutputFcn',  @Worm3DFiducialPickerNetwork_2020_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before Worm3DFiducialPickerNetwork_2020 is made visible.
function Worm3DFiducialPickerNetwork_2020_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Worm3DFiducialPickerNetwork_2020 (see VARARGIN)

% Choose default command line output for Worm3DFiducialPickerNetwork_2020
...
% UIWAIT makes Worm3DFiducialPickerNetwork_2020 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Worm3DFiducialPickerNetwork_2020_OutputFcn(hObject, eventdata, handles)
...
timeOffset_file = [mostRecent filesep 'startWorkspace.mat'];
if exist(timeOffset_file,'file')
    timeOffset=load([mostRecent filesep 'startWorkspace']);
    timeOffset=double(timeOffset.zOffset);
    set(handles.timeOffset,'String',timeOffset);
else
    disp('No startWorkspace file, please tune timeOffset by hand')
...
datFileDir=dir([imFiles filesep 'sCMOS_Frames_U16_*']);
datFile=[imFiles filesep datFileDir.name];
if isempty(Fid)
    Fid=fopen(datFile);
...
if get(handles.channelSelect,'Value')==1
    ...
    activity=temp((rect2(2)+1):(rect2(4)+rect2(2)-1),(1+rect2(1)):(rect2(3)+rect2(1)-1));
...
for iUser=1:length(contents);
    ...
    if ~isempty(currentPoints)
        ...
        if currentTarget <= length(currentFiducials)
            trackedPoint=cell2mat(currentFiducials(currentTarget,:));
        else
            trackedPoint = []
        end
        %trackedPoint
