...
if nargin==0
    ...
    if length(psFolder) == 0
        psFolder=dir([fileparts(dataFolder) filesep 'CLstraight*']);
        imageFolder = [fileparts(dataFolder) filesep psFolder(end).name];
    else
...
for iTime=empty_frames
    ...
    if isfield(pointStats2(iTime), 'stackIdx_ori')
        stackIdx_str = num2str(pointStats2(iTime).stackIdx_ori,'%4.5d');
    else
