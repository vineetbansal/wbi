function clusterWormTrackCompiler_Kevin(filePath)%,fileOutput)

% Modified from Jeff's clusterWormTrackCompiler function to take track
% matrix from Francesco's SMM alignment and give cluster IDs
...
%%
dataFolder=filePath;  %fileparts(filePath);
...
%% load track matrix data
%load pointstats information
pointstats = load([dataFolder filesep 'pointstats.mat']);
pointstats = pointstats.pointstats;
volume_ID = pointstats.stackIdx;
n_volumes = length(volume_ID);

%initialize TrackMatrixi, stackID, and #neuron fields
pointStats_K = {};
for i = 1:n_volumes
    temp = load([dataFolder filesep 'trackMatrix_' num2str(volume_ID(i)) '.mat']);
    pointStats_K(volume_ID(i)+1).TrackMatrixi = double(temp.TrackMatrixi+1);  %[];   %+1 in the index to convert python to MATLAB!~
    pointStats_K(volume_ID(i)+1).stackIdx = volume_ID(i);
    pointStats_K(volume_ID(i)+1).nneurons = pointstats.n_neurons(volume_ID(i)+1);
end

%find which pointStats had tracking completed
track_present = cellfun(@(x) ~isempty(x),{pointStats_K.TrackMatrixi},'uniform',0);
N = find(cell2mat(track_present),1,'last');
display([ 'Number of frames is ' num2str(N)]);

%only go up to last trackmatrix present
presentIdx = 1:length(pointStats_K);

%% prep for matching
TrackMatrix = {pointStats_K(presentIdx).TrackMatrixi};
ref_ID = pointstats.refs;
n_ref_neurons = zeros(1,length(ref_ID));
for rr = 1:length(ref_ID)
    n_ref_neurons(rr) = pointStats_K(ref_ID(rr)).nneurons;
end
...
%% Read .dat files from SMM alignment
% %load track matrix
% fname_track = '/tigress/LEIFER/python-tracking/SMM_track/jobID_refs_0919_100858/Align_jobID_160ref_M_track.dat';
% fileID_track = fopen(fname_track); 
% track = fread(fileID_track,'*ubit64');
% fclose(fileID_track);
% %load volume IDs
% fname_vID = '/tigress/LEIFER/python-tracking/SMM_track/jobID_refs_0919_100858/Align_jobID_160ref_M_vID.dat';
% fileID_vID = fopen(fname_vID);
% vID = fread(fileID_vID,'*ubit64');
% fclose(fileID_vID);
% ref_len = floor(length(track)/length(vID));
% track = reshape(track,floor(length(track)/ref_len), ref_len);
% %load refs
% fname_ref = '/tigress/LEIFER/python-tracking/SMM_track/jobID_refs_0919_100858/Align_jobID_160ref_M_ref.dat';
% fileID_ref = fopen(fname_ref);
% refs = fread(fileID_ref,'*ubit64');
% fclose(fileID_ref);
% %load number of neurons per volume
% load('/tigress/LEIFER/python-tracking/SMM_track/jobID_refs_0919_100858/n_neurons.mat')

%% do matching of tracks by clustering similarities in matching matrices
%find reference size
% ref_neurons = zeros(1,length(refs));
% for r = 1:length(refs)
%     ref_neurons(r) = n_neurons(refs(r),2);
% end
% %build cell array of all track matrices
% TrackMatrix = {};
% for t = 1:size(n_neurons,1)
%     temp = track(t,:)
%     TrackMatrix{t} = reshape();
% end
...
% than 120%
caccumN=find(caccum<nTraining*.4|caccum>nTraining*1.2); %bad clusters   %cluster centers
...
pointStats2_K = pointStats_K;
for i=1:length(pointStats2_K)
    pointStats2_K(presentIdx(i)).trackIdx=track_cell{i};
    pointStats2_K(presentIdx(i)).trackWeights=weight_cell{i};
end
%% SAVE HERE %%
fileOutput_stats=strrep(fileOutput,'.mat','_info.mat');
save(fileOutput,'pointStats2_K');
