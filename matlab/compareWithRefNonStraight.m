...
nRef=length(PS_ref);
%%
TrackMatrixi=zeros(size(P1.rawPoints,1),nRef);
length_P1=size(P1.rawPoints,1);
...
for j_ps=1:length(PS_ref)
    ...
    try
        ...
        T1=[P1.rawPoints P1.Volume.^(1/3) P1.Rintensities];
        T2=[P2.rawPoints P2.Volume.^(1/3) P2.Rintensities];
        %make note of old points
        T1temp=P1.rawPoints(:,1:3);
        T2temp=P2.rawPoints(:,1:3);
        length_P2=size(P2.rawPoints,1);
